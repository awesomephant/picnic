...
client.fetch(query, {}).then((residencies) => {
  ...
  fs.writeFileSync("./src/data/residencies.json", JSON.stringify(sortedResidencies, null, " "))
...
client.fetch(settingsQuery, {}).then((settings) => {
  ...
  fs.writeFileSync("./src/data/settings.json", JSON.stringify(settings[0], null, " "))
